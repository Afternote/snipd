async function createTooltip(e, recreated = false) {
  tooltipStatus = await chrome.storage.local.get("tooltip_enabled");
  console.log(tooltipStatus)
  if (!tooltipStatus.tooltip_enabled) return;
  ...
  setTimeout(function () {
    ...
    if (configs.snapSelectionToWord) {
      if (isTextFieldFocused == true && configs.dontSnapTextfieldSelection == true) {
        ...
        configs.disableWordSnappingOnCtrlKey &&
        e !== undefined &&
    ...
    if (dontShowTooltip == false && selectedText !== null && selectedText !== "") {
      ...
      if (configs.customSearchOptionsDisplay == "hoverCustomSearchStyle")
        setTimeout(function () {
          if (configs.secondaryTooltipEnabled) setHoverForSearchButton(searchButton);
        }, 5);
...
function setUpTooltip(recreated = false) {
  ...
  if (configs.buttonsStyle == "onlyicon" || configs.buttonsStyle == "iconlabel")
...
function calculateTooltipPosition(e) {
  ...
  if (configs.tooltipPosition == "overCursor" && e.clientX < window.innerWidth - 30) {
    ...
    if (
      vertOutOfView ||
      (selEndDimensions.dy - selStartDimensions.dy > 2.0 && selEndDimensions.backwards !== true)
    ) {
      /// show tooltip under selection
      let possibleDyToShowTooltip =
        selEndDimensions.dy + (selEndDimensions.lineHeight ?? 0) + arrow.clientHeight;
    ...
    try {
      ...
      const delta =
        selEndDimensions.dx > selStartDimensions.dx
          ? selEndDimensions.dx - selStartDimensions.dx
          : selStartDimensions.dx - selEndDimensions.dx;

      if (selEndDimensions.dx > selStartDimensions.dx)
        dxToShowTooltip = selStartDimensions.dx + delta / 2;
      else dxToShowTooltip = selEndDimensions.dx + delta / 2;
    } catch (e) {
...
function showTooltip(dx, dy) {
  ...
  if (configs.debugMode) console.log("Selecton tooltip is shown");
  tooltipIsShown = true;
  ...
  if (configs.shiftTooltipWhenWebsiteHasOwn && configs.tooltipPosition !== "overCursor")
    setTimeout(function () {
      ...
      const websiteTooltips = document.querySelectorAll(
        `[style*='position: absolute'][style*='transform'],[class^='popup popup_warning']`
      );
      ...
      if (websiteTooltips !== null && websiteTooltips !== undefined)
        for (let i = 0, l = websiteTooltips.length; i < l; i++) {
          ...
          if (
            ...
            elementClass.toString().includes("selection-tooltip")
          ) {
            ...
            if (
              (elementStyle.includes("position: absolute") &&
                transformStyle !== null &&
                transformStyle !== undefined &&
                transformStyle.includes("translate") &&
              ...
              if (
                ...
                el.clientHeight > 5 &&
                el.clientWidth > 20 &&
                el.getAttribute("id") !== "cmg-fullscreen-image"
              ) {
                if (configs.debugMode) {
...
function hideTooltip(animated = true) {
  ...
  if (oldTooltips !== null && oldTooltips.length !== 0) {
    ...
    if (configs.debugMode) console.log(`Found ${oldTooltips.length} tooltips to hide`);

    for (let i = 0, l = oldTooltips.length; i < l; i++) {
      const oldTooltip = oldTooltips[i];
      if (!animated) oldTooltip.style.transition = "";
    ...
    if (configs.debugMode) console.log("No existing tooltips found");
  }
